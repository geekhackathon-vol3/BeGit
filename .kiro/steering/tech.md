...
iOS クライアント ↔ Cloudflare Workers ↔ Workers Container (Go) ↔ D1 / R2 / FCM / GitHub API

GitHub Webhook でコミット・PR レビューをリアルタイム検知し、FCM 経由でスマホへ Push 通知を送る。
...
- **Push 通知は FCM 経由**: Go バックエンドは FCM HTTP API にポストするだけで APNs の JWT 管理・HTTP/2 接続管理が不要になる。iOS 側には Firebase SDK が必要

...
|---|---|---|---|---|---|
| ① | `begit_time` | グループ全員 | `BeGit Time!` | `今なに作ってる？チームへの通知が届きました` | なし（固定） |
| ② | `nice_work` | 行動した本人 | `Nice Work!` | `いい仕事！写真を撮って投稿しよう` | なし（固定）※ |
...
| ⑦a | `reaction` | 投稿者本人 | `リアクションが届きました` | `{actor_login} があなたの投稿に反応しました` | **`{actor_login}`** を差し込み |
